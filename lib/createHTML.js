...
const createIntern = (intern) => {
    let htmlOutline = fs.readFileSync(
        path.resolve(htmlDirectory, "intern.html"), 'utf-8'
    );
...
const createEngineer = (engineer) => {
    let htmlOutline = fs.readFileSync(
        path.resolve(htmlDirectory, "engineer.html"), 'utf-8'
    );
...
const createMainHtml = (html) => {
    const htmlOutline = fs.readFileSync(
        path.resolve(htmlDirectory, "index.html"), 'utf-8'
    )
